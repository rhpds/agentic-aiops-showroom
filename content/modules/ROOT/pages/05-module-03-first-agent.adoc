...
== Exercise 1: Launch the job and see the gap

The AI/ML team at Meridian Financial has requested Python 3.14 on their RHEL servers. Let's see how the current team handles it.

=== Step 1: Launch the job

. In the **AAP2** tab (log in with `{user}` / `{password}` if prompted), navigate to **Automation Execution -> Templates**

. Launch **10 Install Python 3.14** by clicking the rocket icon image:aap2-rocket-icon.png[rocket icon,20]
+
This playbook attempts to install Python 3.14 from the standard RHEL content — but the package isn't in the default content view for this team's servers. The job will fail quickly.

. Watch the job fail — you'll see an error like:
+
----
...
=== Step 2: Read the Kira ticket

Wait 1-3 minutes for the pipeline to process the failure, then check the result.

. In the **Kira** tab (log in with `{user}` / `{password}` if prompted), look for the new ticket — it will be routed to `sre_linux` since this is a package management failure
