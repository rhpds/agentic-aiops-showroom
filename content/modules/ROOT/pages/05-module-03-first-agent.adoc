...
NOTE: Notice the `lost+found` directory? The PVC, unlike the ConfigMap, appears as a mounted filesystem to the container. You can confirm this with `oc exec deployment/athena --container athena -- df -h`, this makes it simple to add new skills without rebuilding the image, or to iterate on them.
